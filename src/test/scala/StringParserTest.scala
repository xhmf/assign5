  ...
  describe("if else test") {
    object StringParserTestObject extends StringParser {
      def run(): Unit = {
        val node: Node =
          parseStringIntoAST(
            "ITS SHOWTIME BECAUSE IM GOING TO SAY PLEASE I LIED TALK TO THE HAND \"true\" BULLSHIT TALK TO THE HAND \"false\" YOU HAVE NO RESPECT FOR LOGIC YOU HAVE BEEN TERMINATED"
          )
        PrintNodeObject.printNodeNicely(node, 0)
      }
    }
